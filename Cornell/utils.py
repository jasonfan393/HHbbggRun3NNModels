...
def add_PNetCorrections(df):
    ...
    PNetCorr_lead_bjet_pt = df['nonRes_lead_bjet_pt']*(1-df['nonRes_lead_bjet_rawFactor'])*df['nonRes_lead_bjet_PNetRegPtRawCorr']*df['nonRes_lead_bjet_PNetRegPtRawCorrNeutrino']
    PNetCorr_sublead_bjet_pt = df['nonRes_sublead_bjet_pt']*(1-df['nonRes_sublead_bjet_rawFactor'])*df['nonRes_sublead_bjet_PNetRegPtRawCorr']*df['nonRes_sublead_bjet_PNetRegPtRawCorrNeutrino']
    PNetCorr_lead_bjet_mass = df['nonRes_lead_bjet_mass']*(1-df['nonRes_lead_bjet_rawFactor'])*df['nonRes_lead_bjet_PNetRegPtRawCorr']*df['nonRes_lead_bjet_PNetRegPtRawCorrNeutrino']
    PNetCorr_sublead_bjet_mass = df['nonRes_sublead_bjet_mass']*(1-df['nonRes_sublead_bjet_rawFactor'])*df['nonRes_sublead_bjet_PNetRegPtRawCorr']*df['nonRes_sublead_bjet_PNetRegPtRawCorrNeutrino']
    ...
    #correct sumET
    lead_delta_Et = np.sqrt(PNetCorr_lead_bjet_pt**2 + PNetCorr_lead_bjet_mass**2) - np.sqrt(df['nonRes_lead_bjet_pt']**2 + df['nonRes_lead_bjet_mass']**2)
    sublead_delta_Et = np.sqrt(PNetCorr_sublead_bjet_pt**2 + PNetCorr_sublead_bjet_mass**2) - np.sqrt(df['nonRes_sublead_bjet_pt']**2 + df['nonRes_sublead_bjet_mass']**2)
...
def calc_var(df_in,input_var):
    if input_var in df_in.columns:
        raiseException("Should not be using this function for variable already in df")
